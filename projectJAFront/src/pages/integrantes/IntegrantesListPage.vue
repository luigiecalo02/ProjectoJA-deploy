...
function activeOrgs(persona: Persona) {
  return (persona.organizaciones ?? []).filter((o) => o.estado !== false)
}
...
async function confirmDelete(): Promise<void> {
  ...
  try {
    ...
    toast.add({
      ...
      detail: t('integrantes.deleteSuccess'),
      life: 2500,
    })
...
<template>
  <section class="pj-page">
    <header class="pj-page__header">
      <div>
        <h1 class="pj-page__title">{{ t('integrantes.title') }}</h1>
        <p class="pj-page__subtitle">
          {{
            contextOrgLabel
              ? t('integrantes.subtitleScoped', { org: contextOrgLabel })
              : t('integrantes.subtitle')
          }}
        </p>
      </div>
      <Button
        v-if="can('integrantes.create')"
        icon="pi pi-plus"
        :label="t('integrantes.new')"
        @click="router.push({ name: 'integrantes.create' })"
      />
    </header>
      ...
      <template v-else>
        <div class="pj-toolbar" style="margin-bottom: 0.75rem">
          <AppSearchField
            ...
            :placeholder="t('integrantes.searchPlaceholder')"
          />
        </div>
        ...
        >
          <template #empty>
            <p class="pj-muted">{{ t('integrantes.empty') }}</p>
          </template>
          ...
          <Column :header="t('integrantes.clubesAsociados')">
            <template #body="{ data }">
              <div v-if="activeOrgs(data).length" class="orgs-cell">
                <Tag
                  v-for="org in activeOrgs(data)"
                  :key="org.id"
                  severity="info"
                  :value="org.organizacion_nombre || `#${org.organizacion_id}`"
                />
              </div>
              <span v-else class="pj-muted">{{ t('integrantes.noClubes') }}</span>
            </template>
          </Column>
          ...
          <Column :header="t('common.actions')" style="width: 8rem">
            <template #body="{ data }">
              <div class="actions">
                <Button
                  v-if="can('integrantes.update')"
                  icon="pi pi-pencil"
                  text
                  rounded
                  @click="router.push({ name: 'integrantes.edit', params: { id: data.id } })"
                />
                <Button
                  v-if="can('integrantes.delete')"
                  icon="pi pi-trash"
                  text
                  rounded
                  severity="danger"
    ...
    <Dialog
      ...
      :style="{ width: '28rem' }"
    >
      <p>{{ t('integrantes.deleteConfirm') }}</p>
      <template #footer>
