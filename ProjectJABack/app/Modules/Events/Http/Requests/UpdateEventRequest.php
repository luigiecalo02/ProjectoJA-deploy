...
class UpdateEventRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'usar_cabanas' => ['sometimes', 'boolean'],
            ...
            'categoria_ids' => ['sometimes', 'nullable', 'array'],
            'categoria_ids.*' => ['integer', 'exists:categoria_subevento,id'],
            'criterio_disponible_ids' => ['sometimes', 'nullable', 'array'],
            'criterio_disponible_ids.*' => ['integer', 'exists:criterio_evaluacion,id'],
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator) {
            ...
            $event = $this->route('event');
            $min = $this->input('cupo_minimo', $event?->cupo_minimo);
            $max = $this->input('cupo_maximo', $event?->cupo_maximo);
            $ilimitado = array_key_exists('cupo_ilimitado', $this->all())
                ? filter_var($this->input('cupo_ilimitado'), FILTER_VALIDATE_BOOLEAN)
                : (bool) ($event?->cupo_ilimitado ?? false);
