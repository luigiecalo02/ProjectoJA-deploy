...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $name = 'events.create_organization';

        $permissionId = DB::table('permissions')->where('name', $name)->value('id');
        $payload = [
            'display_name' => 'Crear evento en mi organización',
            'module' => 'events',
        ...
        if ($pivot) {
            $roleIds = DB::table('roles')
                ->where(function ($query) {
                    $query->whereIn('name', ['admin', 'director'])->orWhere('is_super', true);
                })
                ->pluck('id');
    ...
    public function down(): void
    {
        ...
        $permissionId = DB::table('permissions')->where('name', 'events.create_organization')->value('id');
        $pivot = Schema::hasTable('permission_role')
            ? 'permission_role'
            : (Schema::hasTable('role_permission') ? 'role_permission' : null);
